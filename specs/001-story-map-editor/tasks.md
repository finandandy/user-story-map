...
- [X] T037 Run the quickstart.md scenarios S1–S5 in a dev vault and confirm `npm test` is green; explicitly open a plugin-authored map in a second vault and confirm it renders identically with no extra setup (FR-015/SC-006); fix any gaps surfaced (final acceptance gate). Done: `npm test` green (55/55), `npm run build` clean, and S1–S5 validated by the maintainer in a live Obsidian vault. S1/S2 surfaced a view-switching gap (racy auto-open + missing Markdown-side toggle button) that was fixed via the `WorkspaceLeaf.setViewState` patch and symmetric header actions before sign-off.
